...
- **Диаграмма Ганта** — задачи во времени, с отметкой заблокированных зависимостями
...
- **Теги** — реестр меток с цветом (как GitHub labels) + разбивка по тегам в дашборде
- **Иерархия и зависимости** — привязка к Epic, «заблокировано задачей» (с защитой от циклов), опциональный авто-старт разблокированных задач и авто-закрытие Epic, когда все дочерние задачи готовы
- **Пауза задачи** — временно приостановить без смены статуса
- **Массовые переходы статуса** — по тегу, с выбором исходного/целевого статуса и предпросмотром совпадений
- **Шаблоны проектов** — сохранить многошаговый план один раз, применять его в один клик как связанный набор задач
- **Тема оформления** — SAP Horizon light/dark + собственный акцентный цвет с проверкой контраста
- **Offline** — данные локально в `data/db/tasks.json`
- **Авто-обновление** — бинарник сам проверяет GitHub Releases и обновляется, не более одного запущенного экземпляра
